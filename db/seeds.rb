# USERS
User.destroy_all
user_info = [
  {
    email: "waden@example.com",
    password: "12341234",
    password_confirmation: "12341234",
    company_name: "novak equipment"
  },
  {
    email: "erikan@example.com",
    password: "12341234",
    password_confirmation: "12341234",
    company_name: "martin implement"
  }
]

user_info.each do |user_hash|
  u = User.new
  u.email = user_hash[:email]
  u.password = user_hash[:password]
  u.password_confirmation = user_hash[:password_confirmation]
  u.company_name = user_hash[:company_name]
  u.save
end

puts "There are now #{User.count} users in the database."

# DEALERSHIPS
dealership_info = [
  {
    address: "801 w ohio st",
    city: "urbana",
    state: "il",
    zip: "69103",
    phone: "7082345678",
    equipment_alley_account: "",
    user_id: "1"
  },
  {
    address: "3451 n w 53rd st",
    city: "champaign",
    state: "il",
    zip: "69144",
    phone: "7089876543",
    equipment_alley_account: "",
    user_id: "1"
  },
  {
    address: "10983 n county ln.",
    city: "san fernando valley",
    state: "ca",
    zip: "99019",
    phone: "9573451092",
    equipment_alley_account: "1234321",
    user_id: "2"
  },
  {
    address: "33 rainbow road",
    city: "san francisco",
    state: "ca",
    zip: "99011",
    phone: "9572349857",
    equipment_alley_account: "9876789",
    user_id: "2"
  }
]

dealership_info.each do |dealership_hash|
  d = Dealership.new
  d.address = dealership_hash[:address]
  d.city = dealership_hash[:city]
  d.state = dealership_hash[:state]
  d.zip = dealership_hash[:zip]
  d.phone = dealership_hash[:phone]
  d.equipment_alley_account = dealership_hash[:equipment_alley_account]
  d.user_id = dealership_hash[:user_id]
  d.save
end

puts "There are now #{Dealership.count} dealerships in the database."

# MACHINES
machine_info = [
  {
    stock_number: "39999",
    year: "2014",
    make: "Hyundai",
    machine_model: "R330",
    machine_type: "Excavator",
    serial_number: "HY345R330678",
    hours: "45",
    price: "99000",
    description: "36 inch tracks, 10 foot boom, 8 foot arm, cab with heater, suspension seat, rearview camera, JRB quick coupler, N&B piping, 60 inch bucket with teeth",
    user_id: "1",
    dealership_id: "1"
  },
  {
    stock_number: "39808",
    year: "2013",
    make: "New Holland",
    machine_model: "L225",
    machine_type: "Skid Steer Loader",
    serial_number: "NDM456789",
    hours: "203",
    price: "45000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, High Flow, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "1",
    dealership_id: "1"
  },
  {
    stock_number: "39800",
    year: "1997",
    make: "New Holland",
    machine_model: "LS170",
    machine_type: "Skid Steer Loader",
    serial_number: "LMU27384949",
    hours: "888",
    price: "19000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, High Flow, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "1",
    dealership_id: "2"
  },
  {
    stock_number: "39700",
    year: "2010",
    make: "Finn",
    machine_model: "T90",
    machine_type: "Hydroseeder",
    serial_number: "8392829282",
    hours: "378",
    price: "49000",
    description: "3 nozzles, 90 hp Kubota diesel engine",
    user_id: "1",
    dealership_id: "2"
  },
  {
    stock_number: "38999",
    year: "1998",
    make: "New Holland",
    machine_model: "L180",
    machine_type: "Skid Steer Loader",
    serial_number: "LMU36282910",
    hours: "1009",
    price: "25000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, High Flow, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "1",
    dealership_id: "1"
  },
  {
    stock_number: "38990",
    year: "1999",
    make: "New Holland",
    machine_model: "LS180",
    machine_type: "Skid Steer Loader",
    serial_number: "LMU36288866",
    hours: "1200",
    price: "28000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, High Flow, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "1",
    dealership_id: "2"
  },
  {
    stock_number: "38900",
    year: "2013",
    make: "New Holland",
    machine_model: "L220",
    machine_type: "Skid Steer Loader",
    serial_number: "NDM234987",
    hours: "250",
    price: "33500",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, 10 x 16.5 tires, 66 inch LPE bucket with bolt-on edge",
    user_id: "1",
    dealership_id: "2"
  },
  {
    stock_number: "38899",
    year: "2010",
    make: "Screen Machine",
    machine_model: "77C",
    machine_type: "Aggregate Screener",
    serial_number: "SM77C92928390",
    hours: "888",
    price: "70000",
    description: "double 3/8 inch wire screens, truck hitch",
    user_id: "1",
    dealership_id: "2"
  },
  {
    stock_number: "38832",
    year: "2011",
    make: "Finn",
    machine_model: "T75",
    machine_type: "Hydroseeder",
    serial_number: "FC839029837",
    hours: "777",
    price: "40000",
    description: "",
    user_id: "1",
    dealership_id: "1"
  },
  {
    stock_number: "38815",
    year: 2010,
    make: "Finn",
    machine_model: "BB302",
    machine_type: "Bark Blower",
    serial_number: "FC839023345",
    hours: "790",
    price: "55000",
    description: "33.5 hp Kubota diesel engine",
    user_id: "2",
    dealership_id: "3"
  },
  {
    stock_number: "38800",
    year: "2009",
    make: "New Holland",
    machine_model: "L190",
    machine_type: "Skid Steer Loader",
    serial_number: "LMU7778202",
    hours: "550",
    price: "40000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, High Flow, 14 x 17.5 tires, 78 inch LPE bucket with bolt-on edge",
    user_id: "2",
    dealership_id: "3"
  },
  {
    stock_number: "38639",
    year: "2013",
    make: "New Holland",
    machine_model: "L230",
    machine_type: "Skid Steer Loader",
    serial_number: "",
    hours: "50",
    price: "",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, 14 x 17.5 tires, 78 inch LPE bucket with bolt-on edge",
    user_id: "2",
    dealership_id: "4"
  },
  {
    stock_number: "38613",
    year: "2014",
    make: "New Holland",
    machine_model: "L225",
    machine_type: "Skid Steer Loader",
    serial_number: "",
    hours: "24",
    price: "",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "2",
    dealership_id: "3"
  },
  {
    stock_number: "37966",
    year: "2000",
    make: "New Holland",
    machine_model: "LS180",
    machine_type: "Skid Steer Loader",
    serial_number: "",
    hours: "4000",
    price: "18000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "2",
    dealership_id: "4"
  },
  {
    stock_number: "37960",
    year: "2008",
    make: "Kubota",
    machine_model: "M59",
    machine_type: "Loader Backhoe",
    serial_number: "",
    hours: "188",
    price: "39493",
    description: "",
    user_id: "2",
    dealership_id: "4"
  },
  {
    stock_number: "36695",
    year: "2013",
    make: "New Holland",
    machine_model: "B95C",
    machine_type: "Loader Backhoe",
    serial_number: "",
    hours: "500",
    price: "105000",
    description: "2-speed drive, cab with heater and A/C, suspension seat, manual quick coupler, block heater, turn signals, 12 x 16.5 tires, 72 inch LPE bucket with bolt-on edge",
    user_id: "2",
    dealership_id: "4"
  },
  {
    stock_number: "36694",
    year: "2013",
    make: "New Holland",
    machine_model: "W80BTC",
    machine_type: "Wheel Loader",
    serial_number: "",
    hours: "275",
    price: "85500",
    description: "Tier 3 engine, standard speed transmission, ROPS cab with heater & A/C, 405/70 R20 STP9 L2 tires/wheels, 1.44 cubic yard bucket with bolt-on cutting edge",
    user_id: "2",
    dealership_id: "3"
  },
  {
    stock_number: "36356",
    year: "2013",
    make: "New Holland",
    machine_model: "C238",
    machine_type: "Compact Track Loader",
    serial_number: "",
    hours: "421",
    price: "59500",
    description: "Tier 3 engine, standard speed transmission, ROPS cab with heater & A/C, 405/70 R20 STP9 L2 tires/wheels, 1.44 cubic yard bucket with bolt-on cutting edge",
    user_id: "2",
    dealership_id: "3"
  },
  {
    stock_number: "36353",
    year: "2013",
    make: "New Holland",
    machine_model: "C238",
    machine_type: "Compact Track Loader",
    serial_number: "",
    hours: "450",
    price: "55000",
    description: "",
    user_id: "2",
    dealership_id: "4"
  },
  {
    stock_number: "35353",
    year: "2012",
    make: "Kubota",
    machine_model: "RTV1140",
    machine_type: "Utility Vehicle",
    serial_number: "",
    hours: "150",
    price: "10500",
    description: "Kubota RTV1140 utility vehicle, 4WD, hydrostatic power steering, front bumper guard, speedometer, hydraulic cargo bed lift, transformable 4-person seating, 25 x 10-12 6 ply HD worksite tires, 24.8 hp Kubota diesel engine",
    user_id: "2",
    dealership_id: "4"
  },
  {
    stock_number: "29134",
    year: "2006",
    make: "New Holland",
    machine_model: "D85WT",
    machine_type: "Dozer",
    serial_number: "",
    hours: "1600",
    price: "55000",
    description: "cab with heater & A/C, radio, hydrostatic drive, 24 inch track pads, 110 inch 6-way blade, auxiliary hydraulics & rear weights",
    user_id: "2",
    dealership_id: "3"
  }
]

machine_info.each do |machine_hash|
  m = Machine.new
  m.stock_number = machine_hash[:stock_number]
  m.year = machine_hash[:year]
  m.make = machine_hash[:make]
  m.machine_model = machine_hash[:machine_model]
  m.machine_type = machine_hash[:machine_type]
  m.serial_number = machine_hash[:serial_number]
  m.hours = machine_hash[:hours]
  m.price = machine_hash[:price]
  m.description = machine_hash[:description]
  m.user_id = machine_hash[:user_id]
  m.dealership_id = machine_hash[:dealership_id]
  m.save
end

puts "There are now #{Machine.count} machines in the database."

# LISTINGS
listing_info = [
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "1"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "1"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "1"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "2"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "2"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "2"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "3"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "3"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "3"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "4"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "4"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "5"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "5"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "6"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "6"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "6"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "7"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "7"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "8"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "8"
  },
  {
    mach_trader: true,
    user_id: "1",
    machine_id: "8"
  },
  {
    equip_alley: true,
    user_id: "1",
    machine_id: "9"
  },
  {
    equip_locator: true,
    user_id: "1",
    machine_id: "9"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "10"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "10"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "10"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "11"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "11"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "12"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "12"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "12"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "13"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "13"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "14"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "14"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "14"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "15"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "15"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "16"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "16"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "16"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "17"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "17"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "18"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "18"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "18"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "19"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "19"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "20"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "20"
  },
  {
    equip_alley: true,
    user_id: "2",
    machine_id: "21"
  },
  {
    equip_locator: true,
    user_id: "2",
    machine_id: "21"
  },
  {
    mach_trader: true,
    user_id: "2",
    machine_id: "21"
  },
]

listing_info.each do |listing_hash|
  l = Listing.new
  l.equip_alley = listing_hash[:equip_alley] if listing_hash[:equip_alley] != nil
  l.equip_locator = listing_hash[:equip_locator] if listing_hash[:equip_locator] != nil
  l.mach_trader = listing_hash[:mach_trader] if listing_hash[:mach_trader] != nil
  l.user_id = listing_hash[:user_id]
  l.machine_id = listing_hash[:machine_id]
  l.save
end

puts "There are now #{Listing.count} listings in the database."
